...
export default function HomePage() {
  ...
  // Safety decision mock stats — in production from /stats/safety
  const safetyAllowed = Math.round(totalJobs * 0.82) || 164;
  const safetyDenied = Math.round(totalJobs * 0.05) || 10;
  const safetyApproval = Math.round(totalJobs * 0.08) || 16;
  const safetyConstrained = Math.round(totalJobs * 0.04) || 8;
  const safetyThrottled = Math.round(totalJobs * 0.01) || 2;
  const safetyTotal = safetyAllowed + safetyDenied + safetyApproval + safetyConstrained + safetyThrottled;
  const safetyAllowRate = safetyTotal > 0 ? Math.round((safetyAllowed / safetyTotal) * 100) : 0;

  // Chart data — Job Activity with Safety Overlay
  const activityData = Array.from({ length: 12 }, (_, i) => {
    ...
    const denied = Math.floor(Math.random() * 8 + 1);
    const approval = Math.floor(Math.random() * 6 + 1);
    return { time: hour, allowed: total - denied - approval, denied, approval };
  });
