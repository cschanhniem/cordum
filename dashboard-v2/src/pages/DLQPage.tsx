...
export default function DLQPage() {
  ...
  const bulkRetryMutation = useMutation({
    mutationFn: async (ids: string[]) => {
      await Promise.allSettled(ids.map((id) => post(`/dlq/${id}/retry`, {})));
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["dlq"] });
      toast.success(`Retrying ${selected.size} items`);
      setSelected(new Set());
      setConfirmBulk(null);
    },
  });
  ...
  const bulkPurgeMutation = useMutation({
    mutationFn: async (ids: string[]) => {
      await Promise.allSettled(ids.map((id) => post(`/dlq/${id}/purge`, {})));
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["dlq"] });
      toast.success(`Purged ${selected.size} items`);
      setSelected(new Set());
      setConfirmBulk(null);
    },
  });
