...
export default function SettingsEnvironmentsPage() {
  const { data: envs, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res: any = await get("/api/environments");
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <PageHeader title="Environments" subtitle="Manage deployment environments and connections" actions={<><Button variant="primary" size="sm" onClick={() => toast.info("Feature coming soon")}>
          ...
          {envs.map((env, i) => (
            <motion.div key={env.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              ...
              <div className="flex gap-2 pt-3 border-t border-border">
                <Button variant="outline" size="sm" className="flex-1" onClick={() => toast.info("Feature coming soon")}>
                  <Shield className="w-3 h-3 mr-1" />Configure
                </Button>
                <Button variant="ghost" size="sm" onClick={() => window.open(env.url, "_blank")}>
