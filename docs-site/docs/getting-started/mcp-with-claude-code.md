...
Shared prerequisites (same for every MCP client):

- `CORDUM_API_KEY` exported from `.env` (or minted via `cordumctl auth login`).
- `CORDUM_TENANT_ID` set (defaults to `default` on single-tenant installs).
- `cordum-mcp` on `$PATH` (stdio transport — cert-free default) OR the gateway reachable over HTTPS with `--cacert ./certs/ca/ca.crt` for self-signed dev deploys.
- Gateway rate limit is 100 req/s per API key; 429 + `Retry-After` on breach.

Full reference with the TLS-trust variants + custom CA bundle recipes: see [`docs/mcp/_prereqs.md`](https://github.com/cordum-io/cordum/blob/main/docs/mcp/_prereqs.md) on GitHub.

![Obtain your CORDUM_API_KEY in the Cordum dashboard](pathname:///img/mcp-claude-code/01-api-key.png)

If the image is a 1×1 placeholder, the screenshot has not yet been captured on a live stack. Human testers capture the real one during the onboarding cold-test (see [`mcp-onboarding-cold-test.md`](https://github.com/cordum-io/cordum/blob/main/docs/mcp/mcp-onboarding-cold-test.md) on GitHub).
...
![`claude --version` prints the current Claude Code release](pathname:///img/mcp-claude-code/02-version.png)
...
![Claude Code lists cordum under connected MCP servers](pathname:///img/mcp-claude-code/03-mcp-servers.png)
...
![Dashboard audit entry for the first tool call](pathname:///img/mcp-claude-code/04-audit-entry.png)
...
- [`mcp-onboarding-cold-test.md`](https://github.com/cordum-io/cordum/blob/main/docs/mcp/mcp-onboarding-cold-test.md) on GitHub — human cold-test checklist (fills in the screenshots above).
